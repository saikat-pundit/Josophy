...
def main():
    print("🚀 Starting parallel openai yield report...")
    ...
    print(f"🔄 Requesting analysis from openai/gpt-oss-120b for {date_str}...")
    ...
    try:
        ...
        # Check for reasoning content and print it to console if present
        ...
        # Capture the final content for the report
        analysis_content = completion.choices[0].message.content

        print("✅ Analysis generated successfully. Saving report...")

        report = f"============================================================\n"
        report += f"📅 YIELD CURVE DAILY REPORT (gpt-oss-120b) - {date_str}\n"
        report += f"============================================================\n\n"
        report += f"📋 AI ANALYSIS:\n{analysis_content}\n"
        report += f"============================================================\n"
        report += f"✅ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | openai/gpt-oss-120b\n"

        report_dir = "reports"
        os.makedirs(report_dir, exist_ok=True)
        report_file = f"{report_dir}/openai_report_{date_str}.txt"
